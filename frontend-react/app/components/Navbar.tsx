...
import { clearSession, getUser, isMR } from '../utils/auth';
  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-white/5"
      ...
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        {/* Brand */}
        <Link to={userIsMR ? '/mr-dashboard' : '/dashboard'} className="flex items-center gap-2.5">
          ...
          <span className="font-bold text-white text-lg tracking-tight">
            Smart<span className="text-blue-400">Pharma</span>
          </span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg glass">
            <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white"
              style={{ background: userIsMR ? 'linear-gradient(135deg, #059669, #10b981)' : 'linear-gradient(135deg, #1d4ed8, #7c3aed)' }}>
              {user?.name?.charAt(0)?.toUpperCase() || 'U'}
            </div>
            <span className="text-sm text-slate-300 font-medium">{user?.name || 'Employee'}</span>
            {userIsMR && <span className="text-xs text-green-400 font-semibold">MR</span>}
          </div>
